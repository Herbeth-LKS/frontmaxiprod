...
function App() {
  const { people, addPerson, deletePerson, loading: peopleLoading, error: peopleError } = usePeople();
  const { addTransaction, loading: transactionsLoading, error: transactionsError } = useTransactions();
  ...
  return (
    <div>
      ...
      {peopleError && <p style={{ color: "red" }}>Erro ao carregar pessoas: {peopleError}</p>}

      {transactionsLoading && <p>Carregando transações...</p>}
      {transactionsError && <p style={{ color: "red" }}>Erro ao carregar transações: {transactionsError}</p>}
